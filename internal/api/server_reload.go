...
func (s *Server) UpdateClientsContext(ctx context.Context, cfg *config.Config) bool {
	...
	if oldCfg == nil || oldCfg.Codex.PlanModelUnsupportedCooldownSeconds != cfg.Codex.PlanModelUnsupportedCooldownSeconds {
		auth.SetCodexPlanModelUnsupportedCooldownSeconds(cfg.Codex.PlanModelUnsupportedCooldownSeconds)
	}
