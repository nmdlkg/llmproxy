...
// Plan labels here mirror the live plan_type values observed on the deployment:
// upstream reports "education", "plus", "pro" and "prolite".
func TestExcludeCodexEducationOnlyModelsGatesAstraForEducationOnly(t *testing.T) {
	tests := []struct {
		name      string
		plan      string
		wantAstra bool
	}{
		{name: "education", plan: "education", wantAstra: false},
		{name: "edu alias", plan: "edu", wantAstra: false},
		{name: "edu mixed case", plan: "Education", wantAstra: false},
		{name: "plus", plan: "plus", wantAstra: true},
		{name: "pro", plan: "pro", wantAstra: true},
		{name: "prolite", plan: "prolite", wantAstra: true},
		{name: "team", plan: "team", wantAstra: true},
		{name: "unknown plan", plan: "future-plan", wantAstra: true},
		{name: "empty plan", plan: "", wantAstra: true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			in := []*internalregistry.ModelInfo{
				{ID: "gpt-6-astra"},
				{ID: "gpt-5.6-sol"},
				{ID: "gpt-5.6-luna"},
			}
			got := excludeCodexEducationOnlyModels(tt.plan, in)
			...
			if foundAstra != tt.wantAstra {
				t.Fatalf("plan %q: astra present = %v, want %v", tt.plan, foundAstra, tt.wantAstra)
			}

			// Models unrelated to the entitlement gap must always survive.
			for _, want := range []string{"gpt-5.6-sol", "gpt-5.6-luna"} {
				found := false
				for _, model := range got {
				...
				if !found {
					t.Fatalf("plan %q unexpectedly dropped %q", tt.plan, want)
				}
			}
		})
	}
...
func TestExcludeCodexEducationOnlyModelsKeepsNonEducationCatalogIdentical(t *testing.T) {
	in := []*internalregistry.ModelInfo{{ID: "gpt-6-astra"}, {ID: "gpt-5.5"}}
	got := excludeCodexEducationOnlyModels("pro", in)
	if len(got) != len(in) {
